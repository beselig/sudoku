import { Coordinates, BoardState, BoardValidityState } from "./types";

// validates Cell, null values are considered invalid
export function getBoardValidityState(state: BoardState) {
...
function validateRow(index: number, state: BoardState, cellValue: number) {
  let matches = 0;
...
function validateCol(index: number, state: BoardState, cellValue: number) {
  let matches = 0;
